...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://esvincreative.in/services",
  },
};
...
export default function ServicesHubPage() {
  ...
  return (
    <main className="min-h-screen bg-white">
            ...
            {services.map((service) => (
              <Link key={service.title} href={service.link} className="group block">
                <div className="h-full bg-white p-8 sketch-border sketch-shadow hover:-translate-y-2 transition-transform duration-300 relative">
                  <div className={`absolute top-0 right-0 w-16 h-16 ${service.color} sketch-border-b sketch-border-l z-0`} />
                  <div className="relative z-10">
                    <h2 className="text-3xl font-bold font-ultra text-black mb-4 group-hover:underline">
                      {service.title}
                    </h2>
                    <p className="text-xl font-ultra text-black/70 mb-8">
                      {service.description}
                    </p>
      ...
      <section className="py-24 bg-highlight-yellow border-t-2 border-dashed border-black text-center px-6">
