    ...
    <main className="min-h-screen bg-gray-50 relative pb-32">
      ...
      <div className="pt-32 pb-12 px-6 max-w-6xl mx-auto">
        ...
        <h1 className="text-4xl md:text-6xl font-bold text-black mb-4">
          <SketchHighlight color="blue">Project Cost Calculator</SketchHighlight>
        </h1>
        <p className="text-xl font-ultra text-black/70">Select your platform and features below to get an instant, itemized project cost estimate.</p>
      </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-black text-white border-t-2 border-black">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-white/50">Estimated Total</div>
            <div className="text-2xl font-bold text-highlight-yellow">{formatPrice(total)}</div>
          </div>
          <button
            disabled={total === 0}
            onClick={() => { setSubmitStatus("idle"); setIsModalOpen(true); }}
            className={`px-5 py-3 font-bold text-base sketch-border transition-colors shrink-0 ${
            total > 0
            ? 'bg-white text-black hover:bg-highlight-pink'
            : 'bg-white/20 text-white/50 cursor-not-allowed'
          }`}>
            Discuss →
          </button>
